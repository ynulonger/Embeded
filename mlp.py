...
if __name__ == '__main__':
	...
	for sub_id in range(1,33):
		...
		for key in sorted(dictionary.keys()):
			if args[3] == "4":
				...
				channels = [1,17,2,19,4,21,3,20,7,25,11,29,13,31]
			mean_accuracy = 0
			feature_index = np.empty(0)
			bands = dictionary[key]

			for channel in channels:
				for band in bands:
					feature_index = np.append(feature_index,channel+(band-1)*32)
			feature_index = list(map(int,feature_index))
			# print(feature_index)
	...
	writer = pd.ExcelWriter("/home/yyl/Embeded/"+args[3]+"_"+with_or_without+"_"+arousal_or_valence+".xlsx")
